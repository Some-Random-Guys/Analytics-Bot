...
class Leaderboard(commands.GroupCog, group_name="leaderboard"):
    ...
    async def letter(self, interaction, user: discord.Member = None):
        ...
        embed.description = "This is a leaderboard of all the letters sent in the server"

        res = await letter_leaderboard(db=db, guild_id=interaction.guild.id, user_id=user.id if user else None)

        if not res:
            await interaction.response.send_message("No letters have been sent in this server", ephemeral=True)
            return
